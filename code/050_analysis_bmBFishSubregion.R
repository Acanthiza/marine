...
#------Data prep------
  ...
  datBM <- datAnimals %>%
    ...
    dplyr::group_by(GSiteID,SurveyID,TransectID,LocalBlock,SurveyDate,SiteName,Subregion,Sanctuary) %>%
    dplyr::summarise(ind = n()
                     , bmBFishSubregion = mean(Biomass) ########## SET Y HERE ##########
                     ) %>%
    dplyr::ungroup() %>%
...
#-----Set values------
  ...
  Y <- "bmBFishSubregion" # dependent variable and used in file names for output
...
#---- Model------
  ...
  if(file.exists(paste0("out/",Y,"/",Y,"_mod.rds")) == FALSE){
    ...
    mod <-
      ...
      stan_glmer(as.formula(paste0("log(",get("Y"),") ~ time*Subregion + (time|SiteName)"))
               , data = datMod
               , family = gaussian()
               #, chains = 6
               #, adapt_delta = 0.99
...
#---- Predict----
  ...
  modPred <- datPred %>%
    ...
    dplyr::mutate(Year = unique(datBM$medYear) + time
                  , raw = value
                  , value = exp(raw)
                  ) %>%
  ...
  modRes <- modPred %>%
    dplyr::group_by(time,Year,Subregion) %>%
    dplyr::summarise(n = n()
...
#------Slope-----
  ...
  modSlope <- modPred %>%
    ...
    dplyr::select(Subregion,row,yearFilter,value) %>%
    tidyr::spread(yearFilter,value) %>%
    dplyr::mutate(diff = max-min)
  
  ggplot(modSlope
         , aes(diff,Subregion,fill=factor(..quantile..))
         ) +
...
#-----Model vs original plot-----
  ...
  ggplot() +
    geom_point(data = datExp
               , aes(Year
                     ,!!ensym(Y)
                     #,colour=!!ensym(legendVar)
                     )
               , alpha = 0.5
               ) +
    # stat_lineribbon(data = modPred
    #                 , aes(x = Year, y = value), .width = c(.95, .80, .50), alpha = 1/4) +
    # 
    # geom_smooth(data = datExp
    #             , aes(Year,!!ensym(Y))
    #             ) +
    ...
    geom_ribbon(data = modRes
                , aes(Year
                      ,ymin=modci90lo
                      ,ymax=modci90up
                      #,fill=!!ensym(legendVar)
                      )
                , alpha = 0.3
                ) +
    facet_wrap(~Subregion) +
    theme(axis.text.x = element_text(angle=90, hjust=0)) +
    coord_cartesian(y = c(0,max(modRes$modMean)*1.1))
  
  ggsave(paste0("out/",Y,"/",Y,"_res_model.png")
         , width = 19
         , height = 28
         , units = "cm"
         )
