...
#-----Set values------
  ...
  Y <- "srSubregion" # dependent variable and used in file names for output
...
#---- Model------
  ...
  if(file.exists(paste0("out/",Y,"/",Y,"_mod.rds")) == FALSE){
    ...
    mod <-
      ...
      stan_glmer(as.formula(paste(get("Y"),"~ time*Subregion*tax + (time|SiteName)"))
               , data = datMod
...
#---- Predict----
  ...
  modRes <- modPred %>%
    dplyr::group_by(time,Year,Subregion,tax) %>%
    dplyr::summarise(n = n()
...
#------Slope-----
  ...
  modSlope <- modPred %>%
    ...
    dplyr::select(Subregion,tax,row,yearFilter,value) %>%
