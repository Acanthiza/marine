...
#------Data prep------
  ...
  datBM <- datAnimals %>%
    ...
    dplyr::summarise(ind = n()
                     , bmBFishSite = mean(Biomass) ########## SET Y HERE ##########
                     ) %>%
    dplyr::ungroup() %>%
...
#-----Set values------
  ...
  Y <- "bmBFishSite" # dependent variable and used in file names for output
...
#---- Model------
  ...
  if(file.exists(paste0("out/",Y,"/",Y,"_mod.rds")) == FALSE){
    ...
    mod <-
      ...
      stan_glm(as.formula(paste0("log(",get("Y"),") ~ time*SiteName"))
               , data = datMod
               , family = gaussian()
               #, chains = 6
               #, adapt_delta = 0.99
