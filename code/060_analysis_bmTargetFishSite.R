...
#------Data prep------
  ...
  datBM <- datAnimals %>%
    ...
    dplyr::group_by(GSiteID,SurveyID,TransectID,SurveyDate,SiteName,CommonName) %>%
    dplyr::summarise(ind = n()
                     , bmTargetFishSite = mean(Biomass) ########## SET Y HERE ##########
                     ) %>%
    dplyr::ungroup() %>%
...
#-----Set values------
  ...
  Y <- "bmTargetFishSite" # dependent variable and used in file names for output
...
#---- Model------
  ...
  if(file.exists(paste0("out/",Y,"/",Y,"_mod.rds")) == FALSE){
    ...
    mod <-
      ...
      stan_glmer(as.formula(paste0(get("Y")," ~ time*SiteName + (1|CommonName)"))
               , data = datMod
               #, family = gaussian()
...
#------Slope-----
  ...
  modSlope <- modPred %>%
    ...
    dplyr::select(SiteName,row,yearFilter,value) %>%
